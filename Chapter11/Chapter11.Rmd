...
qplot(Year, percent, data=plot11.1, geom=c("line", "point"), color=Value, xlab="Year",
...
qplot(Year, Age, data=plot11.3, geom=c("line", "point"), color=Value, xlab = "Year", ylab= "Average age in years") +
  xlim(1820,2100) +
...
plot11.4 <- table.ts11.melted[table.ts11.melted$variable %in% c("Annual inheritance flows", "Adult Mortality"), ]
names(plot11.4) <- c("Year", "Value", "percent")
qplot(Year, percent * 100, data=plot11.4, geom=c("line", "point"), color=Value, xlab = "Year", ylab= "Annual rate of transmission or mortality (%)
") +
  xlim(1820,2010) +
  theme(legend.position=c(0.7, 0.8), legend.background = element_rect(fill="transparent"))

plot11.5 <- table.ts11.melted[table.ts11.melted$variable %in% 
                                c("μ ratio Avg wealth", "μ ratio corrected"), ]
names(plot11.5) <- c("Year", "Value", "percent")
qplot(Year, percent * 100, data=plot11.5, geom=c("line", "point"), color=Value, xlab = "Year", ylab= "Ratio between the average wealth of decedents and the living") + xlim(1820,2010) +
  theme(legend.position=c(0.7, 0.8), legend.background = element_rect(fill="transparent"))
