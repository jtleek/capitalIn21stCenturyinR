...
s6 = read.xlsx("../Piketty2014FiguresTables/Chapter6TablesFigures.xlsx", sheetName="TS6.4", rowIndex=5:120,colIndex=1:2,header=TRUE)

names(s6) =  c('Year','Housing share in national income'
               )
```

```{r fig6.1Clean}
s6 <- melt(s6, id.vars = 'Year')
```

```{r fig6.1Plot, fig.retina=2}
ggplot(data = s6, aes(x=Year, y=value))+
  geom_line(aes(color=variable))+
  geom_point(aes(shape=variable))+
  ...
  ylab('Share of housing rent in national income')+
