...
ggplot(data=ts7.7)+
  ...
  geom_line(aes(x=x,y=continuous,color= 'green')) +
  geom_point(aes(x=x,y=continuous,color= 'green')) +
  ylab('Total income or capital share owned by the poorest x%') +
  xlab('Cumulative x% of population ordered from the poorest to the richest\n \n Curve 1 assumes that the poorest 90% and the richest 10% own 50% of total income or capital each, and that both groups are homogenous \n (hence a linear curve); curve 2 assumes a continuous distribution') + 
