...
# Create euro currency symbol for plot labels
euro <- function(x) {
  paste0("€", format(x, big.mark = ",", decimal.mark = ".", trim = TRUE,
    scientific = FALSE))
} 
# Create yen currency symbol for plot labels
yen <- function(x) {
  paste0("¥", format(x, big.mark = ",", decimal.mark = ".", trim = TRUE,
    scientific = FALSE))
} 
# Create yuan currency symbol for plot labels
...
# French style formatting for yen labels
yenFrench <- function(x) {
  paste0(format(x, big.mark = " ", decimal.mark = ",", trim = TRUE,
    scientific = FALSE), " ¥")
} 
# French style formatting for yuan labels
...
# yen <- yenFrench        # set the custom 'yen' style to custom 'yenFrench'
# yuan <- yuanFrench      # set the custom 'yuan' style to custom 'yuanFrench'
...
rhs <- ggplot(data = subset(df_9_1, variable %in% c("Year", "United States (2013 dollars)")), aes(x = Year, y = value, group = variable, colour = variable, shape = variable)) + geom_line(size = 0, color = 'white', show_guide = FALSE) + xlab(NULL) + ylab("\n Hourly minimum wage (2013 dollars)") + theme_bw() + theme(panel.grid.major = element_line(size = 0.4, linetype = "solid", colour = "White Smoke"), panel.grid.minor = element_blank()) + scale_x_continuous(breaks = NULL) + scale_y_continuous(limits = 1.2*c(1, 10), labels = dollar, breaks = seq(0, 12, by = 1))  # the rhs grid lines overwrite the legend: Tweaked by stopping grid lines before the top.
...
ggplot(data = df_10_5, aes(x = Year, y = value, group = variable, colour = variable, shape = variable)) + geom_line() + geom_point(size = 3) + ylab("Share of top decile or of top percentile in total wealth") + xlab(NULL) + theme_bw() + scale_x_continuous(breaks = seq(1810, 2010, by = 20)) + scale_y_continuous(limits = c(0, 1), labels = percent, breaks = seq(0, 1, by = .1)) + scale_shape_manual(name = "Category", values = shapePalette) + scale_colour_manual(name = "Category", values = colorPalette) + theme(legend.key = element_blank(), legend.position = c(.85, .85), legend.background = element_rect(colour = 'black'), legend.title = element_blank()) 
@
...
ggplot(data = df_10_6, aes(x = Year, y = value, group = variable, colour = variable, shape = variable)) + geom_line() + geom_point(size = 3) + ylab("Share of top decile or of top percentile in total wealth") + xlab(NULL) + theme_bw() + scale_x_continuous(breaks = seq(1810, 2010, by = 20)) + scale_y_continuous(limits = c(0, 1), labels = percent, breaks = seq(0, 1, by = .1)) + scale_shape_manual(name = "Category", values = shapePalettePaired) + scale_colour_manual(name = "Category", values = colorPalette) + theme(legend.key = element_blank(), legend.position = c(.5, .1), legend.background = element_rect(colour = 'black'), legend.title = element_blank()) + guides(shape = guide_legend(ncol = 2))
@
...
ggplot(data = df_10_6S, aes(x = Year, y = value, group = variable, colour = variable, shape = variable)) + geom_line() + geom_point(size = 3) + ylab("Share of top decile or of top percentile in total wealth") + xlab(NULL) + theme_bw() + scale_x_continuous(breaks = seq(1810, 2010, by = 20)) + scale_y_continuous(limits = c(0, 1), labels = percent, breaks = seq(0, 1, by = .1)) + scale_shape_manual(name = "Category", values = shapePalette) + scale_colour_manual(name = "Category", values = colorPalette) + theme(legend.key = element_blank(), legend.position = "bottom", legend.background = element_rect(colour = 'black'), legend.title = element_blank()) + guides(shape = guide_legend(ncol = 2))
@
\caption{Until the mid-twentieth century, wealth inequality was higher in Europe than in the United States.}
\end{minipage}
\end{figure}
...
#ggplot(data = df_11_11, aes(x = Year, y = value, colour = Category)) + geom_line() + geom_point(aes(colour = Category, shape = Category), size = 3) + ylab("Fraction of each cohort") + xlab(NULL) + theme_bw() + scale_x_continuous(breaks = seq(1790, 2030, by = 20)) + scale_y_continuous(labels = percent, breaks = seq(0, .16, by = .02)) + scale_linetype_manual(name = "Fraction", values = linePalette) + scale_shape_manual(name = "Fraction", values = shapePalette) + scale_colour_manual(name = "Fraction", values = colorPalette) + theme(legend.key = element_blank(), legend.position = c(.25, .75), legend.background = element_rect(colour = 'black')) 
@
